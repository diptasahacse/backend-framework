import { and, or, sql, SQLWrapper } from "drizzle-orm";
import z from "zod";
...
export type OrderDirectionType = typeof OrderDirection[number];
  ...
  private static processRule(rule: FilterRule): SQLWrapper {
    ...
    switch (rule.operator) {
      ...
      case Operators.IN:
        ...
        return sql`${column} = ANY(ARRAY(${rule.value}))`;
      ...
      case Operators.BETWEEN:
        if (!Array.isArray(rule.value) || rule.value.length !== 2) {
          throw new Error("Invalid BETWEEN rule value");
        }
        return sql`${column} BETWEEN ${rule.value[0]} and ${rule.value[1]}`;
      case Operators.NOT_BETWEEN:
        if (!Array.isArray(rule.value) || rule.value.length !== 2) {
          throw new Error("Invalid NOT BETWEEN rule value");
        }
